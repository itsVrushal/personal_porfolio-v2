...
function ColabLink({ url }: { url: string }) {
  return (
    <a
      href={url}
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-1.5 font-mono text-[11px] px-3 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/25 hover:bg-amber-500/20 hover:border-amber-500/50 hover:shadow-[0_0_15px_rgba(245,158,11,0.25)] transition-all duration-300 group/colab flex-shrink-0"
      title="Open interactive notebook in Google Colab"
    >
      ...
      <span>Colab</span>
      <span className="text-[10px] group-hover/colab:translate-x-0.5 group-hover/colab:-translate-y-0.5 transition-transform duration-200">
        ↗
      </span>
    </a>
  );
              ...
              return (
                <button
                  ...
                  className={`font-mono text-xs px-3.5 py-1.5 rounded-full transition-all duration-300 flex items-center gap-2 cursor-pointer ${
                    isActive
                      ? "bg-accent text-bg font-semibold shadow-[0_0_15px_rgba(45,212,191,0.35)]"
                      : "bg-surface/70 text-text-lo hover:text-text-hi hover:bg-surface border border-border"
                  }`}
                >
                  <span>{cat}</span>
                  <span
                    className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                      isActive ? "bg-bg/25 text-bg font-bold" : "bg-border/60 text-text-lo"
                    }`}
                  >
                    {count}
                  </span>
                        ...
                        <div className="md:col-span-5 flex flex-col justify-between">
                          <div>
                            <div className="flex items-center justify-between gap-3 mb-3">
                              ...
                              {project.colab && <ColabLink url={project.colab} />}
                            </div>
